...
# --- 入力ファイル ---
BASE_CSV_PATH = '250724/data/exapmle_matched.csv'
...
# --- 出力ファイル ---
FINAL_ROU_FILE_PATH = f'250724/data/example_added_v2_scenario_{SCENARIO_ID}.rou.xml'
...
add_rou_list_2_1_1 = [
    ["128185343", "Anywhere", 1500], ["1231325634#1", "Anywhere", 1500],
    ["Anywhere", "128185343", 1500], ["Anywhere", "1231325634#1", 1500]
]
# ... (他のadd_rou_list_*も同様にここに定義)
...
try:
    ...
    for vehicle in pool_tree.getroot().findall('vehicle'):
        ...
        if route is not None and 'edges' in route.attrib:
            ...
            if len(edges) > 1:
                # キーを (出発エッジ, 終点エッジ) のタプルとする
                od_pair = (edges[0], edges[-1])
                validated_pool[od_pair].append(vehicle)
    print(f"経路プールから {len(validated_pool)} 種類のODペア、合計 {sum(len(v) for v in validated_pool.values())} 台の有効な経路を読み込みました。")
except FileNotFoundError:
    print(f"エラー: 検証済み経路プールファイル '{VALIDATED_POOL_PATH}' が見つかりません。")
    print("事前に generate_trip_pool.py と duarouter を実行してください。")
    exit()

# --- 2. シナリオに基づき、プールからトリップをサンプリング ---
final_added_vehicles = []
total_added_count = 0
...
if selected_scenario:
    for v_type, demand_list in selected_scenario.items():
        ...
        for i, (o_base, d_base, count) in enumerate(demand_list):
            # プールからこのOD定義に合致する全経路を集める
            candidate_vehicles = []
            if o_base != "Anywhere" and d_base != "Anywhere": # ODが両方固定
                candidate_vehicles.extend(validated_pool.get((o_base, d_base), []))
            elif o_base != "Anywhere": # 出発地のみ固定
                for (o, d), vehicles in validated_pool.items():
                    if o == o_base: candidate_vehicles.extend(vehicles)
            elif d_base != "Anywhere": # 目的地のみ固定
                for (o, d), vehicles in validated_pool.items():
                    if d == d_base: candidate_vehicles.extend(vehicles)
            ...
            sampled_indices = np.random.choice(len(candidate_vehicles), count, replace=True)
            
            for k, index in enumerate(sampled_indices):
                # 元のプールに影響が出ないように、選択したビークルを完全にコピーする
                original_vehicle = candidate_vehicles[index]
                vehicle_to_add = copy.deepcopy(original_vehicle)
                
                # ID, depart, typeをシナリオに合わせて上書き
                vehicle_to_add.set('id', f't_add_{v_type}_{i}_{k}')
                vehicle_to_add.set('depart', str(np.random.randint(depart_min, depart_max)))
                if v_type_name:
                    vehicle_to_add.set('type', v_type_name)
                elif 'type' in vehicle_to_add.attrib:
                    del vehicle_to_add.attrib['type'] # 普通車の場合はtype属性を削除
                
                final_added_vehicles.append(vehicle_to_add)
            
            total_added_count += len(sampled_indices)

print(f"シナリオ {SCENARIO_ID} のために、{total_added_count} 台の追加トリップをサンプリングしました。")
...
try:
    ...
    if num_days > 0:
        ...
        # 運行ID単位でランダムにサンプリングする関数
        def sample_by_vehicle_id(df_source, target_count):
            if df_source.empty or target_count == 0: 
    ...
    for _, row in df_mini.iterrows():
        ...
        trip.set('id', f't_base_{row["rou_id"]}') # IDが重複しないようにプレフィックスを追加
        if row['自動車の種別'] == 1:
            trip.set('type', 'truck')
...
# 元のトリップを追加 (base_trips_to_add)
for trip in base_trips_to_add: rou_root.append(trip)

# サンプリングした追加トリップ（vehicle要素）を追加
for vehicle in final_added_vehicles:
    rou_root.append(vehicle)

# 必要であれば、rou_rootの子要素をdepartでソート
all_elements = sorted(list(rou_root), key=lambda x: int(x.get('depart')))
rou_root[:] = all_elements
