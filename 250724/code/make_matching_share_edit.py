    ...
    def format_output(self, rou_file_path):
        ...
        for i, single_demand in enumerate(trips_temp):
            ...
            # トラック以外の場合
                ...
                person = ET.SubElement(rou_root, 'person')
                person.set('id', f't_{single_demand[0]}')
                person.set('depart', str(single_demand[3]))
                person.set('period', '1')
                
                person_trip = ET.SubElement(person, 'personTrip')
                if single_demand[1][-1] == 'N':
                    person_trip.set('fromJunction', single_demand[1][:-1])
                else:
                    person_trip.set('from', single_demand[1])
                if single_demand[2][-1] == 'N':
                    person_trip.set('toJunction', single_demand[2][:-1])
                else:
                    person_trip.set('to', single_demand[2])
...
def sample_random_trips(n, trips_df_csv="./trips_df.csv", random_trips_df_csv="random_trips_df.csv"):
    total_rows = sum(1 for _ in open(trips_df_csv)) - 1
    n = min(n, total_rows)
