...
# --- 3. 【新規】ボックスプロットによる外れ値の可視化 ---
plt.figure(figsize=(8, 6))
sns.boxplot(data=df[['tl_per_rl_s1', 'tl_per_rl_s4']])
plt.title('単位長さあたりTimeLossの分布（外れ値除去前）')
...
bins = np.arange(np.floor(min_val / 100) * 100, np.ceil(max_val / 100) * 100 + 100, 100)
...
# 棒グラフ（差の合計値）を描画
ax1.bar(neg_data['midpoint'], neg_data['sum'], width=50, color='dodgerblue', alpha=0.7)
ax1.bar(pos_data['midpoint'], pos_data['sum'], width=50, color='orangered', alpha=0.7)
...
# グラフの装飾
ax1.set_title('TimeLossの差（合計値と頻度）の分布（外れ値除去後）', fontsize=16, fontweight='bold')
